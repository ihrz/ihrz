const { QuickDB } = require('quick.db'), db = new QuickDB(), DiscordOauth2 = require("discord-oauth2"), oauth = new DiscordOauth2(), logger = require(`${process.cwd()}/files/core/logger`);

module.exports = async (req, res) => {
    const { userid, tor, adminKey } = req.body;

    if (!userid || !adminKey) return logger.warn("-> Bad json request without ip/key");
    if (!tor == 'CHECK_IN_SYSTEM') { logger.warn('-> Bad json requests without options'); return res.send('-> Bad json requests without options') };
