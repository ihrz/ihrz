...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        let wl_channel = await db.get(`Here4CreateChannels_${interaction.guild.id}`)
        ...
        let d0de = await db.get(`customchanstatus_${interaction.guild.id}_${interaction.user.id}`)
        if (d0de === undefined || d0de === null){ return interaction.reply("you dont have channels !")}
    
            let nameChan = await db.get(`customchanID_${interaction.guild.id}_${interaction.user.id}`)
