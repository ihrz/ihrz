...
export async function footerBuilder(message) {
...
export async function footerAttachmentBuilder(interaction) {
    var res = await displayBotPP(interaction instanceof Client
        ?
            interaction
        :
            interaction.client, interaction instanceof Guild
        ?
            interaction.id
        :
...
export async function displayBotPP(client, guildId) {
    let botPFP = await db.get(`${guildId}.BOT.botPFP`);
    if (!botPFP) {
...
;
export default async function displayBotName(guildId) {
    let botName = await db.get(`${guildId}.BOT.botName`);
    if (!botName) {
