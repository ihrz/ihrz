...
*/
import { EmbedBuilder, time, ButtonBuilder, ButtonStyle, ActionRowBuilder, ChatInputCommandInteraction, } from 'discord.js';
...
class GiveawayManager {
    ...
    constructor(client, options) {
        ...
        client.on('interactionCreate', async (interaction) => {
            if (interaction.isButton() && interaction.customId === "confirm-entry-giveaway") {
                await this.addEntries(interaction);
            }
            ;
        });
        this.refresh(client);
                ...
                switch (giveawayData?.requirement.type) {
                    ...
                    case "messages":
                        reqPass = (await interaction.client.
                            ...
                            || []).length >= parseInt(giveawayData.requirement.value);
                        break;
                    case "roles":
                        reqPass = interaction.member?.roles.cache.has(giveawayData.requirement.value);
                        break;
                }
                ;
                if (!reqPass) {
    ...
    async finish(client, giveawayId, guildId, channelId) {
        ...
        if (!fetch.ended || fetch.ended === 'End()') {
            ...
            let winners = winner ? winner.map((winner) => `<@${winner}>`).join(",") : null;
