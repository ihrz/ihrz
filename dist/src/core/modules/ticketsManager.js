...
*/
import { EmbedBuilder, ButtonBuilder, ButtonStyle, ActionRowBuilder, ChannelType, PermissionsBitField, UserSelectMenuBuilder, ButtonInteraction, StringSelectMenuInteraction, ComponentType, TextInputStyle, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, ChannelSelectMenuBuilder, } from 'discord.js';
...
async function CreateButtonPanel(interaction, data) {
...
;
async function CreateSelectPanel(interaction, data) {
    let lang = await interaction.client.func.getLanguageData(interaction.guildId);
    let case_list = [];
        ...
        if (i.customId === "remove_selection") {
            ...
            if (case_list.length > 0) {
                ...
                await og_interaction.edit({
                    components: case_list.length === 0 ? [og_interaction.components[0]] : [og_interaction.components[0], new ActionRowBuilder().addComponents(comp)]
                });
            }
        }
            ...
            await og_interaction.edit({ components: [button, new ActionRowBuilder().addComponents(comp)] });
    ...
    async function sendCategorySelection(interaction, x) {
        const action_row_category = new ActionRowBuilder()
    ...
    async function reasonTicket(interaction) {
        ...
        const i_category = await interaction.client.method.channelSend(interaction.message, {
            ...
            components: [new ActionRowBuilder().addComponents(action_row_category)]
        });
        const response = await i_category?.awaitMessageComponent({
            componentType: ComponentType.StringSelect,
            time: 30399999
...
;
async function CreateTicketChannelV2(interaction) {
    let panelCode = await interaction.client.db.get(`${interaction.guildId}.GUILD.TICKET_PANEL.${interaction.message.id}`);
    let result = await interaction.client.db.get(`${interaction.guildId}.GUILD.TICKET_PANEL.${panelCode}`);
    ...
    await interaction.guild?.channels.create({
        ...
        if (interaction instanceof StringSelectMenuInteraction) {
            embeds.push(new EmbedBuilder()
                .setColor(2829617)
                .setDescription(lang.sethereticket_panel_select_embed_desc
...
;
async function CreateChannelV2(interaction, result) {
    let lang = await interaction.client.func.getLanguageData(interaction.guildId);
    let values = [];
    let reasonInteraction;
    let category = result.config.optionFields.find(item => item.value === interaction.values[0])?.categoryId
        || result.category;
...
;
async function TicketReOpen(interaction) {
    let data = await interaction.client.func.getLanguageData(interaction.guildId);
    let fetch = await database.get(`${interaction.guildId}.TICKET_ALL`);
    for (let user in fetch) {
        for (let channel in fetch[user]) {
...
;
async function TicketDelete(interaction) {
    let data = await interaction.client.func.getLanguageData(interaction.guildId);
    let fetch = await database.get(`${interaction.guildId}.TICKET_ALL`);
    for (let user in fetch) {
        for (let channel in fetch[user]) {
...
;
async function TicketAddMember_2(interaction) {
    let data = await interaction.client.func.getLanguageData(interaction.guildId);
    ;
    ...
    interaction.channel.permissionOverwrites.cache
        .filter((i) => i.type === 1).each((i) => { listmembersArray.push(i.id); });
    let addedMembers = [];
    let removedMembers = [];
...
;
export { CreateButtonPanel, CloseTicket, CreateTicketChannel, CreateTicketChannelV2, CreateSelectPanel, TicketTranscript, TicketDelete, TicketRemoveMember, TicketAddMember, TicketReOpen, TicketAddMember_2, };
