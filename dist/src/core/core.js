...
import { Collection, DefaultWebSocketManagerOptions } from 'discord.js';
...
export async function main(client) {
    ...
    client.config.owner.owners?.forEach(owner => {
        if (!Number.isNaN(Number.parseInt(owner)))
            client.owners.push(owner);
    });
    if (!Number.isNaN(client.config.owner.ownerid1))
        client.owners.push(client.config.owner.ownerid1);
    if (!Number.isNaN(Number.parseInt(client.config.owner.ownerid2)))
        client.owners.push(client.config.owner.ownerid2);
    errorManager.uncaughtExceptionHandler(client);
    assetsCalc(client);
    emojis(client);
    client.db = DatabaseModel;
    client.bash = new Collection();
    ...
    client.vanityInvites = new Collection();
