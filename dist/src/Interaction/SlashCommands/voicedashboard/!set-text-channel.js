...
export default {
    run: async (client, interaction, lang, command, neededPerm) => {
        ...
        let components = buttonRows.map(row => new ActionRowBuilder().addComponents(...row.map(button => new ButtonBuilder()
            .setEmoji(button.emoji)
            .setCustomId(button.customId)
            .setStyle(ButtonStyle.Secondary)
            .setDisabled(button.disabled || false))));
        let response = await targetedChannel.send({ embeds: [embed], components });
