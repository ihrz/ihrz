...
*/
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ChannelSelectMenuBuilder, ChannelType, ComponentType, EmbedBuilder, PermissionsBitField, TextChannel, } from 'discord.js';
export default {
    run: async (client, interaction, lang, command, neededPerm) => {
        // Guard's Typing
        if (!interaction.member || !client.user || !interaction.member.user || !interaction.guild || !interaction.channel)
            return;
        ...
        ;
        var baseData = await client.db.get(`${interaction.guildId}.GUILD.GUILD_CONFIG`);
        ...
        let embed = new EmbedBuilder()
            ...
            .setTimestamp()
            .addFields({ name: lang.setchannels_embed_fields_value_join, value: current_join_channel, inline: true }, { name: lang.setchannels_embed_fields_value_leave, value: current_leave_channel, inline: true });
        let action_row = new ActionRowBuilder()
            ...
            if (i.customId === 'guildconfig-channel-panel-change-join-channel') {
                ...
                i2Collector?.on('collect', async (result) => {
                    ...
                    if (!(channel instanceof TextChannel)) {
                        ...
                        try {
                            ...
                            ;
                            interaction.guild.channels.cache.get(channelId).send({ content: lang.setchannels_confirmation_message_on_join });
                            ...
                            embed.setFields({ name: lang.setchannels_embed_fields_value_join, value: current_join_channel, inline: true }, { name: lang.setchannels_embed_fields_value_leave, value: current_leave_channel, inline: true });
                ...
                i2Collector?.on('collect', async (result) => {
                    ...
                    try {
                        ...
                        ;
                        interaction.guild.channels.cache.get(channelId).send({ content: lang.setchannels_confirmation_message_on_leave });
                        await client.db.set(`${interaction.guildId}.GUILD.GUILD_CONFIG.leave`, channelId);
                        i2.delete();
                        i2Collector.stop();
                        await result.reply({
