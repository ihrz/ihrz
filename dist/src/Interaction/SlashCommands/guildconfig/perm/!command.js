...
export default {
    run: async (client, interaction, lang, command, neededPerm) => {
        ...
        if (choice === "change") {
            ...
            const existingPerms = await client.db.get(`${interaction.guildId}.UTILS.PERMS.${requestedCommand}`);
            ...
            if (existingPerms) {
                if (typeof existingPerms === 'object' && 'roles' in existingPerms) {
                    ...
                    if (removedRoles.length > 0) {
                        changes.push(`${lang.perm_rmv_role}: ${removedRoles.map(roleId => {
                ...
                if (typeof existingPerms === 'object' && 'users' in existingPerms) {
                    ...
                    if (removedUsers.length > 0) {
                        changes.push(`${lang.perm_rmv_usr}: ${removedUsers.map(userId => {
            ...
            const res = await client.db.get(`${interaction.guildId}.UTILS.PERMS`);
