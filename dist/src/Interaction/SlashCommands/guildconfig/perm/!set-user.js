...
export default {
    run: async (client, interaction, lang, command, neededPerm) => {
        ...
        let perm = interaction.options.getString('permission');
        if (perm === "0") {
            ...
            let fetchedPerm = await client.method.permission.checkUserPermissions(interaction.member);
            // @ts-ignore
            if (Array.isArray(fetchedPerm) ? false : fetchedPerm <= parseInt(perm) && interaction.guild.ownerId !== interaction.member.id) {
                await client.method.interactionSend(interaction, {
                    content: lang.perm_set_warn_message.replace("${interaction.member.toString()}", interaction.member.toString()),
                });
                return;
            }
