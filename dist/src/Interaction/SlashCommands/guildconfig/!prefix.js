...
export default {
    run: async (client, interaction, lang, command, neededPerm) => {
        ...
        if (action === "mention") {
            ...
            if (!prefix)
                return await interaction.editReply({ content: lang.guildconfig_setbot_prefix_prefix_specify_prefix });
            if (prefix.length >= 5)
                return await interaction.editReply({ content: lang.guildconfig_setbot_prefix_prefix_too_long });
