...
export default {
    run: async (client, interaction, lang, command, neededPerm) => {
        ...
        let baseData = await client.db.get(`${interaction.guildId}`);
        const pages = [];
        ...
        const generateEmbedForFields = async (fields) => {
            const embed = new EmbedBuilder()
                .setColor(await client.db.get(`${interaction.guild?.id}.GUILD.GUILD_CONFIG.embed_color.all`) || "#016c9a")
                .setDescription(lang.guildprofil_embed_description.replace(/\${interaction\.guild\.name}/g, interaction.guild?.name))
                .addFields(fields)
...
function supportConfigToString(supportConfig, lang) {
...
function ticketFetchedToString(charForTicket, lang) {
    let ticketFetched = '';
...
function reactionRolesToString(charForRr, guild, lang) {
    let reactionrole = '';
    for (const i in charForRr) {
        ...
        if (a) {
            let stringContent = Object.keys(a).map((key) => {
                ...
                const emoji = guild?.emojis.cache.find((emoji) => emoji.id === key);
...
function xpStatsToString(xp, lang) {
...
function logsToString(logs, lang) {
...
function blockBotToString(blockBot, lang) {
    return blockBot ? lang.guildprofil_blockbot_on : lang.guildprofil_blockbot_off;
}
function picOnlyChannelsToString(picOnly, lang) {
    return (picOnly === undefined) || (picOnly.length === 0) ? lang.var_none : picOnly?.map((x) => `<#${x}>`).join(",");
}
function picOnlyConfigToString(picOnlyConfig, lang) {
...
function tooNewAccountToString(user, tooNewConfig, lang) {
...
function roleSaverToString(rs, lang) {
...
function voiceDashToString(vc, lang) {
