...
*/
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, EmbedBuilder, PermissionsBitField, RoleSelectMenuBuilder, ComponentType, PermissionFlagsBits } from 'discord.js';
export default {
    run: async (client, interaction, lang, command, neededPerm) => {
        ...
        let all_channels = await client.db.get(`${interaction.guildId}.GUILD.GUILD_CONFIG`);
        ...
        if (all_channels?.joinroles !== undefined && all_channels?.joinroles.length > 1) {
            const roles = Array.isArray(all_channels.joinroles) ? all_channels.joinroles : [all_channels.joinroles];
        ...
        let saveButton = new ButtonBuilder()
            ...
            .setEmoji('💾');
        let comp = new ActionRowBuilder().addComponents(roleSelectMenu);
        let comp_2 = new ActionRowBuilder().addComponents(saveButton);
        let og_response = await client.method.interactionSend(interaction, {
            embeds: [embed],
            components: [comp, comp_2]
        });
        const confirmedDangerousRoles = new Set();
        const tooHighterRoles = new Set();
        let selectedRoles = [];
        let all_roles = [];
        ...
        collector.on('collect', async (roleInteraction) => {
            ...
            for (const role of roleInteraction.roles) {
                ...
                const rolePermissions = new PermissionsBitField(role[1].permissions);
        ...
        buttonCollector.on('collect', async (buttonInteraction) => {
            ...
            if (buttonInteraction.customId === 'guildconfig-joinRoles-save-button') {
                let newComp_2 = new ActionRowBuilder()
        ...
        function updateEmbed(embed, roles, lang) {
        ...
        ;
        async function handleDangerousRolesConfirmation(roleInteraction, dangerous_roles, confirmedDangerousRoles, embed, og_response, lang) {
            ...
            let confirm_buttons = new ActionRowBuilder()
                ...
                .setLabel(lang.mybot_submit_utils_msg_yes), new ButtonBuilder()
        ...
        async function handleTooHighterRoles(roleInteraction, too_highter_roles) {
