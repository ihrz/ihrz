...
export default async function (interaction) {
    ...
    if (result.channelId !== interaction.channelId)
        return await interaction.deferUpdate();
    function getPreviousOwner(guild) {
        var result = '';
    ...
    if (targetedChannel?.members.get(previousOwner?.id))
        return await interaction.deferUpdate();
    if (!member.voice.channel) {
        ...
        targetedChannel?.setName(lang.temporary_voice_channel_name.replace("{nicker}", `${interaction.user.displayName || interaction.user.username}`));
        targetedChannel?.permissionOverwrites.delete(previousOwner?.user.id);
