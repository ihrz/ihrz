...
*/
import { EmbedBuilder, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, ActionRowBuilder, ComponentType, ChatInputCommandInteraction, ApplicationCommandType, ApplicationCommandOptionType, ButtonStyle, ButtonBuilder, } from 'discord.js';
...
async function updatePage(message, embeds, currentPage, menuRows, navigationRow) {
...
async function handleCategorySelect(i, response, categories, menuRows, client, lang, bot_prefix, Commands) {
    const previousCollector = response.buttonCollector;
    ...
    response.buttonCollector = buttonCollector;
...
export const command = {
    ...
    run: async (client, interaction, lang, command, neededPerm, args) => {
        ...
        if (!targetCommand) {
            ...
            const Commands = await client.db.get(`${interaction.guildId}.UTILS.PERMS`);
            let index = 0;
            for (let i = 0; i < 2; i++) {
                ...
                categoriesCalc.forEach((category) => {
                    selectMenu.addOptions(new StringSelectMenuOptionBuilder()
                        ...
                        .setDescription(lang.help_select_menu_fields_desc.replace("${categories[index].value.length}", category.value.length.toString()))
            ...
            collector.on('collect', async (i) => {
                ...
                await i.deferUpdate();
                await handleCategorySelect(i, response, categories, rows, client, lang, bot_prefix, Commands);
            });
