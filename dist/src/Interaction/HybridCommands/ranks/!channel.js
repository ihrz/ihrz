...
export default {
    run: async (client, interaction, lang, command, neededPerm, args) => {
        ...
        if (interaction instanceof ChatInputCommandInteraction) {
            ...
            var argsid = interaction.options.getChannel("channel");
        }
        else {
        ...
        if (type === "on") {
            ...
            try {
                ...
                if (already === argsid.id)
                    return await client.method.interactionSend(interaction, { content: lang.setxpchannels_already_with_this_config });
                interaction.guild.channels.cache.get(argsid.id).send({ content: lang.setxpchannels_confirmation_message });
