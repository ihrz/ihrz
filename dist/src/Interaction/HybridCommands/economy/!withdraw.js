...
export default {
    run: async (client, interaction, lang, command, neededPerm, args) => {
        ...
        if (!interaction.member || !client.user || !interaction.guild || !interaction.channel)
            return;
        let dataAccount = await client.db.get(`${interaction.guildId}.USER.${interaction.member.user.id}.ECONOMY`);
        if (interaction instanceof ChatInputCommandInteraction) {
            var toWithdraw = interaction.options.getString('how-much');
        }
        else {
            var toWithdraw = client.method.string(args, 0);
        }
        ;
        ...
        ;
        if (toWithdraw === "all")
            toWithdraw = dataAccount.bank?.toString();
        ...
        let embed = new EmbedBuilder()
            .setAuthor({ name: lang.daily_embed_title, iconURL: interaction.member.user.displayAvatarURL() })
