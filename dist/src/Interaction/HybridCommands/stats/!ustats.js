...
import { calculateMessageTime, calculateVoiceActivity, calculateActiveChannels, calculateActiveVoiceChannels, getChannelName, getChannelMessagesCount, getChannelMinutesCount, } from "../../../core/functions/userStatsUtils.js";
export default {
    run: async (client, interaction, lang, command, args) => {
        // Guard's Typing
        if (!client.user ||
            !interaction.member ||
            !interaction.guild ||
        ...
        if (interaction instanceof ChatInputCommandInteraction) {
            ...
            member = (client.method.member(interaction, args, 0) || interaction.member);
            user = interaction.author;
        }
        let res = (await client.db.get(`${interaction.guildId}.STATS.USER.${member.user.id}`));
        if (!res) {
        ...
        let weeklyMessages = [];
        let dailyMessages = [];
        let totalMessages = res.messages?.length || 0;
        ...
        res.messages?.forEach((msg) => {
            const result = calculateMessageTime(msg, nowTimestamp, dailyTimeout, weeklyTimeout, monthlyTimeout, dailyMessages, weeklyMessages, monthlyMessages);
        ...
        res.voices?.forEach((voice) => {
            const result = calculateVoiceActivity(voice, nowTimestamp, dailyTimeout, weeklyTimeout, monthlyTimeout, dailyVoiceActivity, weeklyVoiceActivity, monthlyVoiceActivity);
