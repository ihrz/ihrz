...
export default {
    run: async (client, interaction, lang, command, neededPerm, args) => {
        ...
        ;
        if (guildMember) {
            if (interaction.member.roles.highest.position <= guildMember.roles.highest.position) {
                await client.method.interactionSend(interaction, {
        ...
        const ban = async () => {
            interaction.guild?.bans.create(member?.id, { reason: `Banned by: ${(interaction.member?.user).globalName || interaction.member?.user.username} | Reason: ${reason}` })
                .then(async () => {
                client.method.interactionSend(interaction, {
                    embeds: [
                        new EmbedBuilder()
                            .setTitle(lang.setjoinroles_var_perm_ban_members)
                            .setFields({ name: lang.var_member, value: member.toString(), inline: true }, { name: lang.var_author, value: interaction.member?.toString(), inline: true }, { name: lang.var_reason, value: reason || lang.var_no_set, inline: true })
