...
*/
import { EmbedBuilder, PermissionsBitField, ChatInputCommandInteraction, ActionRowBuilder, ButtonBuilder, ButtonStyle, } from 'discord.js';
...
export default {
    run: async (client, interaction, lang, command, neededPerm, args) => {
        ...
        ;
        let allWarns = await client.db.get(`${interaction.guildId}.USER.${member?.id}.WARNS`);
        if (!allWarns) {
            await client.method.interactionSend(interaction, {
