...
export default {
    run: async (client, interaction, lang, command, neededPerm, args) => {
        ...
        if (interaction instanceof ChatInputCommandInteraction) {
            ...
            var user2 = await client.method.user(interaction, args, 1) || interaction.guild?.members.cache.random()?.user;
        }
        ...
        try {
            ...
            let always100 = client.config.command.alway100;
            var found = always100.find(element => {
                if (element === `${user1.id}x${user2.id}`
                    ||
