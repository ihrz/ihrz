...
export default {
    run: async (client, interaction, lang, command, neededPerm, args) => {
        ...
        if (interaction instanceof ChatInputCommandInteraction) {
            var victim = interaction.options.getUser("user");
        ...
        function generateRandomIPv4() {
            let generateOctet = () => Math.floor(Math.random() * 256);
        ...
        let embed = new EmbedBuilder()
            ...
            .addFields({ name: lang.hack_embed_fields_ip, value: `\`${generatedIp}\`` }, { name: lang.hack_embed_fields_email, value: `\`${generatedEmail}\`` }, { name: lang.hack_embed_fields_password, value: `\`${generatedPassword}\`` }, { name: "📞", value: `\`${generatedPhoneNumber}\`` }, { name: "🔑", value: `\`${bruteForce()}\`` }, { name: "🏠", value: `\`${getAddress()}\`` }, { name: "💳", value: `\`${getCreditCards()}\`` })
            .setTimestamp();
