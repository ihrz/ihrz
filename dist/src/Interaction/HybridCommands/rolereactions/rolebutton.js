...
*/
import { PermissionsBitField, ApplicationCommandOptionType, ChatInputCommandInteraction, ApplicationCommandType, ButtonBuilder, ButtonStyle, ChannelType } from 'discord.js';
export const command = {
    ...
    run: async (client, interaction, lang, command, neededPerm, args) => {
        ...
        if (type == "add") {
            if (!role) {
                return await client.method.interactionSend(interaction, { content: lang.buttonreaction_roles_not_found });
            }
            ;
            if (!reaction) {
                return await client.method.interactionSend(interaction, { content: lang.reactionroles_missing_reaction_added });
            }
            ;
            await channel?.messages.fetch(messagei)
                .then(async (msg) => {
            ...
            await channel?.messages.fetch(messagei)
                .then(async (message) => {
                let res = await client.db.get(`${interaction.guildId}.GUILD.REACTION_ROLES.${message.id}`);
