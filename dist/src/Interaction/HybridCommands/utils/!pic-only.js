...
*/
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, EmbedBuilder, PermissionsBitField, ComponentType, PermissionFlagsBits, ChannelSelectMenuBuilder, ChannelType, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, TextInputStyle } from 'discord.js';
...
export default {
    run: async (client, interaction, lang, command, neededPerm, args) => {
        ...
        let all_channels = await client.db.get(`${interaction.guildId}.UTILS.picOnly`) || [];
        let baseData = await client.db.get(`${interaction.guildId}.UTILS.picOnlyConfig`) || {
            threshold: 3,
            muteTime: 600000
        };
        ...
        let choiceSelectMenu = new StringSelectMenuBuilder()
            ...
            .setValue("utils-picOnly-option-change-time"), new StringSelectMenuOptionBuilder()
        ...
        if (all_channels !== undefined && all_channels?.length > 1) {
            const channels = Array.isArray(all_channels) ? all_channels : [all_channels];
        ...
        let saveButton = new ButtonBuilder()
            ...
            .setEmoji('💾');
        let comp = new ActionRowBuilder().addComponents(channelSelectMenu);
        let comp_1 = new ActionRowBuilder().addComponents(choiceSelectMenu);
