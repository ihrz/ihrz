...
*/
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ChatInputCommandInteraction, EmbedBuilder, PermissionFlagsBits, PermissionsBitField, } from 'discord.js';
export default {
            ...
            if (interaction_2.customId === 'previousPage') {
                ...
                if (interaction_2.user.id === interaction_2.guild?.ownerId) {
                    ...
                    for (let member of to_unrank_members) {
                        let filtered_roles = Array.from(member.roles.cache
                            .filter(x => !x.permissions.has(PermissionFlagsBits.Administrator))
                            .keys());
                        try {
