...
*/
import { EmbedBuilder, PermissionsBitField, ActionRowBuilder, ComponentType, StringSelectMenuBuilder, ButtonBuilder, ButtonStyle, StringSelectMenuOptionBuilder, ChatInputCommandInteraction, ChannelSelectMenuBuilder, ChannelType, ChannelSelectMenuInteraction, } from 'discord.js';
...
export default {
    run: async (client, interaction, lang, command, neededPerm, args) => {
        ...
        let select = new StringSelectMenuBuilder()
            ...
            .addOptions(new StringSelectMenuOptionBuilder().setLabel(lang.embed_placeholder_option_copy_embed).setEmoji("📥").setValue('0'), new StringSelectMenuOptionBuilder().setLabel(lang.embed_placeholder_option_edit_title).setEmoji("🖊").setValue('1'), new StringSelectMenuOptionBuilder().setLabel(lang.embed_placeholder_option_delete_title).setEmoji("💥").setValue('2'), new StringSelectMenuOptionBuilder().setLabel(lang.embed_placeholder_option_edit_description).setEmoji("💬").setValue('3'), new StringSelectMenuOptionBuilder().setLabel(lang.embed_placeholder_option_delete_description).setEmoji("📝").setValue('4'), new StringSelectMenuOptionBuilder().setLabel(lang.embed_placeholder_option_edit_author).setEmoji("🕵️").setValue('5'), new StringSelectMenuOptionBuilder().setLabel(lang.embed_placeholder_option_delete_author).setEmoji("✂").setValue('6'), new StringSelectMenuOptionBuilder().setLabel(lang.embed_placeholder_option_edit_footer).setEmoji("🔻").setValue('7'), new StringSelectMenuOptionBuilder().setLabel(lang.embed_placeholder_option_delete_footer).setEmoji("🔺").setValue('8'), new StringSelectMenuOptionBuilder().setLabel(lang.embed_placeholder_option_edit_thumbnail).setEmoji("🔳").setValue('9'), new StringSelectMenuOptionBuilder().setLabel(lang.embed_placeholder_option_edit_image).setEmoji("🖼️").setValue('10'), new StringSelectMenuOptionBuilder().setLabel(lang.embed_placeholder_option_edit_titleurl).setEmoji("🌐").setValue('11'), new StringSelectMenuOptionBuilder().setLabel(lang.embed_placeholder_option_edit_color).setEmoji("🎨").setValue('12'), new StringSelectMenuOptionBuilder().setLabel(lang.embed_placeholder_option_delete_color).setEmoji("🔵").setValue('13'));
        ...
        let response = await client.method.interactionSend(interaction, {
            ...
            components: [
                ...
                new ActionRowBuilder().addComponents(save, send, replace, cancel)
            ],
        });
                ...
                case '0':
                    await handleCollector(i, 'embed_choose_0', async (message) => {
                        try {
                            ...
                            const channel = interaction.guild?.channels.cache.get(parts.channelId);
                            if (!channel) {
        ...
        async function handleCollector(i, replyContent, onCollect) {
            const replyMessage = Array.isArray(lang[replyContent]) ? lang[replyContent].join(' ') : lang[replyContent];
            let reply = await i.reply({ content: replyMessage.toString(), ephemeral: true });
            let messageCollector = interaction.channel?.createMessageCollector({ filter: (m) => m.author.id === interaction.member?.user.id, max: 1, time: 300_000 });
        ...
        async function replaceEmbed(confirmation) {
            await confirmation.update({
                content: lang.embed_replace_question_msg,
                components: [],
                files
            });
            let response2 = await interaction.channel?.awaitMessages({
        ...
        async function sendEmbed(confirmation) {
            ...
            seCollector?.on('collect', async (result) => {
                if (result instanceof ChannelSelectMenuInteraction) {
                    ...
                    if (!channel)
                        return;
                    await channel.send({ embeds: [__tempEmbed], files });
                    seCollector.stop();
                    await response.edit({
        ...
        buttonCollector.on('collect', async (confirmation) => {
            ...
            switch (confirmation.customId) {
                case "save":
                    if (arg)
                        await client.db.delete(`EMBED.${arg}`);
        ...
        function extractDiscordUrlParts(url) {
            try {
