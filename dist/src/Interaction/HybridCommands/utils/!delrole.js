...
export default {
    run: async (client, interaction, lang, command, neededPerm, args) => {
        ...
        ;
        let allowed_roles = await client.db.get(`${interaction.guildId}.UTILS.wlRoles`);
        ...
        ;
        if (interaction.member.roles.highest.position <= user.roles.highest.position && interaction.member.user.id !== user.id) {
            await client.method.interactionSend(interaction, {
