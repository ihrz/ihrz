...
export default {
    run: async (client, interaction, lang, command, neededPerm, args) => {
        // Guard's Typing
        if (!interaction.member || !client.user || !interaction.member || !interaction.guild || !interaction.channel)
            return;
        ...
        let all_roles = await client.db.get(`${interaction.guildId}.UTILS.wlRoles`) || [];
        ...
        if (all_roles !== undefined && all_roles?.length > 1) {
            const roles = Array.isArray(all_roles) ? all_roles : [all_roles];
