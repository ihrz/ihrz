...
*/
import { StringSelectMenuBuilder, StringSelectMenuOptionBuilder, EmbedBuilder, ActionRowBuilder, ComponentType, TextInputStyle, ApplicationCommandType } from 'discord.js';
...
export const command = {
    ...
    run: async (client, interaction, lang, command, neededPerm, options) => {
        ...
        async function chooseAction(i) {
            ...
            async function __1(arg0) {
                ...
                if (!fetched || !fetched[arg0]) {
                    ...
                    let embed = new EmbedBuilder()
                        ...
                        iconURL: (interaction.member?.user).displayAvatarURL({ extension: 'png', size: 512 })
                    })
                ...
                for (let i in fetched) {
                    embed.addFields({
                        name: `#${i}`, value: lang.schedule_list_fields_embed
                            .replace("${date.format(new Date(fetched[i]?.expired), 'YYYY/MM/DD HH:mm:ss')}", format(new Date(fetched[i]?.expired), 'YYYY/MM/DD HH:mm:ss'))
            ...
            ;
            async function executeAfterModal(i) {
                ...
                async function __0(date0, collection) {
