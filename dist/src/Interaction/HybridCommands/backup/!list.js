...
export default {
    run: async (client, interaction, lang, command, neededPerm, args) => {
        ...
        ;
        let data2 = await client.db.get(`BACKUPS.${interaction.member.user.id}`);
        let backups = [];
        for (let i in data2) {
            let result = data2[i];
            ...
            if (result)
                backups.push({ name: `${result.guildName} - (||${i}||)`, value: v });
        }
        ;
        ...
        const generateEmbed = async (page) => {
            let embed = new EmbedBuilder()
                ...
                .setAuthor({ name: interaction.member?.user.username || interaction.member?.displayName, iconURL: "attachment://user_icon.png" })
        ...
        const generateButtons = (page) => {
            return new ActionRowBuilder()
                ...
                .setDisabled(page === 0 || backups.length === 0), new ButtonBuilder()
                .setCustomId('next')
                .setLabel('>>')
                .setStyle(ButtonStyle.Secondary)
        ...
        const originalResponse = await client.method.interactionSend(interaction, {
            ...
            files: [
                ...
                { attachment: await interaction.client.func.image64(interaction.member.user.displayAvatarURL()), name: 'user_icon.png' }
            ]
        });
        if (backups.length > 0) {
