...
export const event = {
    ...
    run: async (client, oldState, newState) => {
        const baseData = await client.db.get(`${newState.guild.id}.UTILS.LEASH`);
        ...
        const matchedPairings = validEntries.filter(x => x.sub === oldState.member?.id ||
            x.dom === oldState.member?.id ||
            x.sub === newState.member?.id ||
            x.dom === newState.member?.id);
        for (const pairing of matchedPairings) {
            const subMembers = pairing.sub.split(',').map(id => newState.guild.members.cache.get(id.trim())).filter(member => member !== undefined);
            const domMember = newState.guild.members.cache.get(pairing.dom);
            if (!domMember || subMembers.length === 0)
                continue;
