...
export const event = {
    ...
    run: async (client, oldPresence, newPresence) => {
        if (!newPresence.guild?.members.me?.permissions.has([PermissionsBitField.Flags.ManageRoles]))
            return;
        if (!oldPresence || !oldPresence.guild)
            return;
        ...
        ;
        if (bio.state?.toString().toLowerCase().includes(someinfo.input.toString().toLowerCase())
