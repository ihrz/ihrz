...
export const event = {
    ...
    run: async (client, message) => {
        if (message.author.bot)
            return;
        const picOnlyChannels = await client.db.get(`${message.guildId}.UTILS.picOnly`);
        const picOnlyConfig = await client.db.get(`${message.guildId}.UTILS.picOnlyConfig`);
            ...
            if (!hasValidImageAttachment) {
                ...
                if (userWarnings.length >= 3) {
                    ...
                    try {
                        ...
                        await client.method.warnMember(message.guild?.members.me, message.member, "Automated Punishment - Pic Only").catch(() => { });
