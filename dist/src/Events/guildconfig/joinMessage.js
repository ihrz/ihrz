...
*/
import { AttachmentBuilder, GuildFeature, PermissionsBitField, SnowflakeUtil } from 'discord.js';
export async function generateJoinImage(member, optionalOptions) {
    var htmlContent = member.client.htmlfiles["guildconfigWelcomeCart"];
    var ImageBannerOptions = await member.client.db.get(`${member.guild.id}.GUILD.GUILD_CONFIG.joinbanner`);
...
export const event = {
    ...
    run: async (client, member) => {
        ...
        let invite = newInvites.find((i) => i.uses && i.uses > (oldInvites?.get(i.code) || 0));
        ...
        let ImageBannerStates = await client.db.get(`${member.guild.id}.GUILD.GUILD_CONFIG.joinbannerStates`);
        let channel = member.guild.channels.cache.get(wChan);
        let files = [];
