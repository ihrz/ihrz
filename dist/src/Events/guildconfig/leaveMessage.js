...
export const event = {
    ...
    run: async (client, member) => {
        ...
        try {
            ...
            if (!lChan || !member.guild.channels.cache.get(lChan))
                return;
            let lChanManager = member.guild.channels.cache.get(lChan);
            if (base?.inviter) {
                const inviter = await client.users.fetch(base.inviter);
                const inviterStats = await client.db.get(`${member.guild.id}.USER.${inviter.id}.INVITES`);
                if (inviterStats) {
                ...
                const messageContent = client.method.generateCustomMessagePreview(leaveMessage || data.event_goodbye_inviter, {
                    user: member.user,
                    guild: member.guild,
                ...
                await lChanManager.send({
                    content: client.method.generateCustomMessagePreview(leaveMessage ||
