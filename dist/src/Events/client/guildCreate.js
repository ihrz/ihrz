...
*/
import { Collection, EmbedBuilder, PermissionsBitField, ActionRowBuilder, ButtonBuilder, ButtonStyle, ChannelType } from 'discord.js';
...
export const event = {
    ...
    run: async (client, guild) => {
        if (!guild)
            return;
        let highestPositionChannel = null;
        ...
        async function blacklistLeave() {
            let channelHr = guild.channels.cache.get(guild.systemChannelId)
        ...
        async function messageToServer() {
            ...
            let embed = new EmbedBuilder()
                ...
                .setDescription(`## ${welcomeMessage[Math.floor(Math.random() * welcomeMessage.length)]}\n` +
        ...
        async function ownerLogs() {
            ...
            let embed = new EmbedBuilder()
                ...
                .addFields({ name: "🏷️・Server Name", value: `\`${guild.name}\``, inline: true }, { name: "🆔・Server ID", value: `\`${guild.id}\``, inline: true }, { name: "🌐・Server Region", value: `\`${guild.preferredLocale}\``, inline: true }, { name: "👤・Member Count", value: `\`${guild.memberCount}\` members`, inline: true }, { name: "🔗・Invite Link", value: `\`${await createInvite(channel)}\``, inline: true }, { name: "🪝・Vanity URL", value: `\`${i || "None"}\``, inline: true }, { name: "🍻・New guilds total", value: client.guilds.cache.size.toString(), inline: true }, { name: "🥛・New members total", value: `${usersize} members`, inline: true })
        ...
        if (d)
            ownerLogs(), messageToServer(), getInvites();
    },
};
