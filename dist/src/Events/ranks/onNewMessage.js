...
*/
import { PermissionsBitField, ChannelType, SnowflakeUtil } from 'discord.js';
...
export const event = {
    ...
    run: async (client, message) => {
        ...
        let guildLocal = await client.db.get(`${message.guild.id}.GUILD.LANG.lang`) || "fr-FR";
        if ((await parseMessageCommand(client, message)).success)
            return;
        ...
        var ranksConfig = await client.db.get(`${message.guild.id}.GUILD.XP_LEVELING`);
        var xpTurn = ranksConfig?.disable;
        if (xpTurn === 'disable' || ranksConfig?.bypassChannels?.includes(message.channelId))
            return;
        ...
        if ((level * 500) < baseData?.xp) {
            ...
            if (xpTurn === false
                || !message.channel.permissionsFor(client.user)?.has(PermissionsBitField.Flags.SendMessages))
                return;
            let xpChan = ranksConfig?.xpchannels;
            let MsgChannel = message.guild.channels.cache.get(xpChan);
