...
export const event = {
    ...
    run: async (client, oldState, newState) => {
        ...
        let category_channel = newState.guild.channels.cache.get(result_channel?.parentId);
        ...
        if (oldState.channel?.members.size === 0 && allChannel) {
            ...
            for (let [userId, channelId] of allChannelEntries) {
                if (channelId !== oldState.channelId)
                    continue;
                let userChannel = newState.guild.channels.cache.get(channelId);
            ...
            newState.guild.channels.create({
                ...
                newState.member?.voice.setChannel(chann.id)
                    ...
                    if ((await chann.fetch()).members.size === 0) {
                        ...
                        chann.permissionOverwrites.edit(newState.member?.user.id, {
                            ViewChannel: true,
                            Connect: true,
                            Stream: true,
                            Speak: true,
