...
export function getDetailedDiff(oldText, newText) {
...
function getDifferenceIndex(oldLine, newLine) {
    let diffIndex = -1;
...
function getTruncatedLine(line, diffIndex) {
    const maxLineLength = 30;
...
export const event = {
    ...
    run: async (client, oldMessage, newMessage) => {
        ...
        if (!oldMessage || !oldMessage.guild)
            return;
        ...
        if (oldMessage.content.length > 160 || newMessage.content.length > 160) {
            logsEmbed.setFields({ name: data.var_message, value: getDetailedDiff(oldMessage.content, newMessage.content) });
        }
        else {
            logsEmbed.setFields({ name: data.event_srvLogs_messageUpdate_footer_1, value: oldMessage.content + '.' }, { name: data.event_srvLogs_messageUpdate_footer_2, value: newMessage.content + '.' });
        }
