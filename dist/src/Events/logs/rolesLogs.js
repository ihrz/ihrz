...
export const event = {
    ...
    run: async (client, oldMember, newMember) => {
        ...
        let newObjectsnewObjectIds = newObjects.map((obj) => obj.id);
        let removeObjectIds = removeObjects.map((obj) => obj.id);
        let logsEmbed = new EmbedBuilder()
            .setColor(await client.db.get(`${oldMember.guild?.id}.GUILD.GUILD_CONFIG.embed_color.audits-logs`) || "#000000")
            .setAuthor({ name: firstEntry.target?.username, iconURL: firstEntry.target?.displayAvatarURL({ extension: 'png', forceStatic: false, size: 512 }) })
            .setTimestamp();
        let desc = ' ';
