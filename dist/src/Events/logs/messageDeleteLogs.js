...
export const event = {
    ...
    run: async (client, message) => {
        ...
        if (message.attachments.size >= 1) {
            ...
            if (attachments.size >= 2) {
                ...
                async function getFile(file) {
                    let response = await axios.get(file['attachment'], { responseType: 'arrayBuffer' });
                ...
                let snipedImage;
                await axios.get(attachment['attachment'], { responseType: 'arrayBuffer' }).then((response) => {
        ...
        await Msgchannel.send({ embeds: [logsEmbed, ...saves_emb] }).catch(() => { });
        return;
    },
};
