...
*/
import { EmbedBuilder, PermissionsBitField, AuditLogEvent, PermissionFlagsBits } from 'discord.js';
export const event = {
    name: "guildBanRemove",
    run: async (client, ban) => {
        let data = await client.func.getLanguageData(ban.guild.id);
        if (!ban.guild.members.me
            || !ban.guild.members.me.permissions.has(PermissionsBitField.Flags.ViewAuditLog))
            return;
