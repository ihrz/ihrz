...
export const subCommand: SubCommand = {
	run: async (
		...
		let all_roles: DatabaseStructure.GuildConfigSchema["joinroles"] =
			(await client.db.get(
				`${interaction.guildId}.GUILD.GUILD_CONFIG.joinroles`
			)) || [];
		...
		buttonCollector.on(
			...
			async (buttonInteraction: ButtonInteraction) => {
				...
				if (
					...
					"guildconfig-joinRoles-save-button"
				) {
					const newComp_2 =
						...
						components: [
							new ActionRowBuilder<ButtonBuilder>().addComponents(
								new ButtonBuilder()
									...
									.setStyle(ButtonStyle.Secondary)
									.setCustomId(
										"guildconfig-joinRoles-choice-timeOut"
									)
