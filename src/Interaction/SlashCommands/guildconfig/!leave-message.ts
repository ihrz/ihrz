...
export const subCommand: SubCommand = {
	run: async (
		...
		let leaveMessage = await client.db.get(
			`${interaction.guildId}.GUILD.GUILD_CONFIG.leavemessage`
		);
		const guildLocal =
		...
		collector.on("collect", async (buttonInteraction) => {
			...
			if (buttonInteraction.customId === "leaveMessage-set-message") {
				...
				await modalInteraction.reply({
					content:
						lang.setleavemessage_command_work_on_enable.replace(
				...
				await client.func.ihorizon_logs(interaction, {
					...
					description:
						lang.setleavemessage_logs_embed_description_on_enable.replace(
							"${interaction.user.id}",
							interaction.member.user.id
						)
				});
			} else if (
				buttonInteraction.customId === "leaveMessage-default-message"
			) {
				const newEmbed = EmbedBuilder.from(helpEmbed).setFields({
					name: lang.setjoinmessage_help_embed_fields_custom_name,
				...
				await client.func.ihorizon_logs(interaction, {
					...
					description:
						lang.setleavemessage_logs_embed_description_on_disable.replace(
							"${interaction.user.id}",
							interaction.member.user.id
						)
				});
