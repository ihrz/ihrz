...
function hasPermissionRequirements(
	permData: DatabaseStructure.PermCommandData
): boolean {
	return (
		permData.users.length > 0 ||
		...
		if (choice === "change") {
			...
			if (!requestedCommand) {
				await client.func.method.interactionSend(interaction, {
					content: lang.perm_add_args_error
				});
				return;
			}
			...
			if (perms) {
				...
				const normalizedLevel =
					permLevel === 0
						? null
						: (permLevel as DatabaseStructure.PermLevel);
			...
			if (customRole) {
				...
				if (roleIndex === -1) {
					changes.push(
						`${lang.perm_set_add_role}: ${customRole.toString()}`
					);
					newPerms.roles.push(customRole.id);
				} else {
					changes.push(
						`${lang.perm_rmv_role}: ${customRole.toString()}`
					);
			...
			if (customUser) {
				...
				if (userIndex === -1) {
					changes.push(
						`${lang.perm_set_add_usr}: ${customUser.toString()}`
					);
					newPerms.users.push(customUser.id);
				} else {
					changes.push(
						`${lang.perm_rmv_usr}: ${customUser.toString()}`
					);
			...
			const res = (await client.db.get(
				`${interaction.guildId}.UTILS.PERMS`
			)) as DatabaseStructure.UtilsPermsData;

			if (!res || Object.keys(res).length === 0) {
				await client.func.method.interactionSend(interaction, {
					content: lang.perm_list_no_command_set
				});
				return;
			}
			...
			for (const [commandName, permData] of Object.entries(res)) {
				if (typeof permData === "number") {
					...
					const normalizedPermData =
						normalizePermissionData(permData);
			...
			const message = (await client.func.method.interactionSend(
				interaction,
				{
			...
			if (commands) {
				const existingPerms = (await client.db.get(
					`${interaction.guildId}.UTILS.PERMS.${commands}`
				)) as
					| DatabaseStructure.PermCommandData
					| DatabaseStructure.PermLevel
					| undefined;
				...
				if (!existingPerms) {
					await client.func.method.interactionSend(interaction, {
						content: lang.perm_command_delete_dont_exist
					});
					return;
				}
				...
				await client.func.method.interactionSend(interaction, {
					content: lang.perm_command_delete_command_deleted.replace(
						"${commands}",
						commands
					)
				});
				...
				await client.func.method.interactionSend(interaction, {
					content: lang.perm_command_delete_specify_command
				});
				return;
			}
			...
			if (perms) {
				for (const [commandName, permData] of Object.entries(
					existingPerms
				)) {
					if (typeof permData === "number") {
						if (permData === parseInt(perms)) {
							await client.db.delete(
								`${interaction.guildId}.UTILS.PERMS.${commandName}`
							);
							changes.push(
								`- ${commandName} (${lang.var_level}: ${permData})\n`
							);
						}
					} else {
						const normalizedPermData =
						...
						if (normalizedPermData.level === parseInt(perms)) {
							...
							changes.push(
								`- ${commandName} (${lang.var_level}: ${normalizedPermData.level})\n`
							);
						}
					}
				}
			...
			if (customRole) {
				for (const [commandName, permData] of Object.entries(
					...
					if (typeof permData !== "number") {
						const normalizedPermData =
							normalizePermissionData(permData);
						const roleIndex = normalizedPermData.roles.indexOf(
							customRole.id
						);
						if (roleIndex !== -1) {
							normalizedPermData.roles.splice(roleIndex, 1);
							const nextPermData =
								normalizePermissionData(normalizedPermData);
							...
							changes.push(
								`- ${commandName} (${lang.var_roles}: @${customRole.name})\n`
							);
						}
					}
				}
			...
			if (customUser) {
				for (const [commandName, permData] of Object.entries(
					...
					if (typeof permData !== "number") {
						const normalizedPermData =
							normalizePermissionData(permData);
						const userIndex = normalizedPermData.users.indexOf(
							customUser.id
						);
						if (userIndex !== -1) {
							...
							changes.push(
								`- ${commandName} (${lang.var_user}: ${customUser.id})\n`
							);
						}
					}
				}
			...
			if (changes.length === 0) {
				await client.func.method.interactionSend(interaction, {
					content: lang.perm_command_delete_all_zero_change
				});
				return;
			} else {
				let msg = `\`\`\`diff\n${changes.join("")}\`\`\``;
