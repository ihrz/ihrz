...
export const subCommand: SubCommand = {
	run: async (
		...
		const baseData = (await client.db.get(
			`${interaction.guildId}`
		)) as DatabaseStructure.DbInId;
		const pages: EmbedBuilder[] = [];

		const joinDmMessageField = {
			name: lang.guildprofil_embed_fields_joinDmMessage,
			value: baseData?.GUILD?.GUILD_CONFIG?.joindm
				? "```" +
					baseData?.GUILD?.GUILD_CONFIG?.joindm.substring(0, 1020) +
					"```"
				: lang.guildprofil_not_set_joinDmMessage
		};
		const joinMessageField = {
			name: lang.guildprofil_embed_fields_joinmessage,
			value: baseData?.GUILD?.GUILD_CONFIG?.joinmessage
				? "```" + baseData?.GUILD?.GUILD_CONFIG?.joinmessage + "```"
				: lang.guildprofil_not_set_joinmessage
		};
		const leaveMessageField = {
			name: lang.guildprofil_embed_fields_leavemessage,
			value: baseData?.GUILD?.GUILD_CONFIG?.leavemessage
				? "```" + baseData?.GUILD?.GUILD_CONFIG?.leavemessage + "```"
				: lang.guildprofil_not_set_leavemessage
		};

		const setChannelsLeaveField = {
			name: lang.guildprofil_embed_fields_setchannelsleave,
			value: baseData?.GUILD?.GUILD_CONFIG?.leave
				? `<#${baseData?.GUILD?.GUILD_CONFIG?.leave}>`
				: lang.guildprofil_not_set_setchannelsleave
		};
		const setChannelsJoinField = {
			name: lang.guildprofil_embed_fields_setchannelsjoin,
			value: baseData?.GUILD?.GUILD_CONFIG?.join
				? `<#${baseData?.GUILD?.GUILD_CONFIG?.join}>`
				: lang.guildprofil_not_set_setchannelsjoin
		};

		const joinRolesField = {
			name: lang.guildprofil_embed_fields_joinroles,
			value:
				Array.isArray(baseData?.GUILD?.GUILD_CONFIG?.joinroles) &&
				baseData?.GUILD?.GUILD_CONFIG?.joinroles.length > 0
					? baseData?.GUILD?.GUILD_CONFIG?.joinroles
							.map((x) => `<@&${x}>`)
							.join(", ")
					: lang.guildprofil_not_set_joinroles
		};
		const blockPubField = {
			name: lang.guildprofil_embed_fields_blockpub,
			value:
				baseData?.GUILD?.GUILD_CONFIG?.antipub === "on"
					? lang.guildprofil_set_blockpub
					: lang.guildprofil_not_set_blockpub
		};

		const punishPubField = {
			name: lang.guildprofil_embed_fields_punishPub,
			value: baseData?.GUILD?.PUNISH?.PUNISH_PUB
				? lang.guildprofil_set_punishPub
						.replace(
							/\${punishPub\.punishementType}/g,
							baseData.GUILD?.PUNISH?.PUNISH_PUB
								?.punishementType ?? ""
						)
						.replace(
							/\${punishPub\.amountMax}/g,
							String(
								baseData.GUILD?.PUNISH?.PUNISH_PUB?.amountMax ??
									0
							)
						)
				: lang.guildprofil_not_set_punishPub
		};
		...
		const reactionRoleField = {
			...
			value: reactionRolesToString(
				baseData?.GUILD?.REACTION_ROLES,
				interaction.guild,
				lang
			)
		};
		...
		const joinGhostpingField = {
			...
			value: joinGhostpingToString(
				baseData.GUILD?.GUILD_CONFIG?.GHOST_PING,
				lang
			)
		};
		...
		const tooNewAccountField = {
			...
			value: tooNewAccountToString(
				interaction.member,
				baseData.GUILD?.BLOCK_NEW_ACCOUNT,
				lang
			)
		};
		...
		const roleSaverField = {
			...
			value: roleSaverToString(
				baseData.GUILD?.GUILD_CONFIG?.rolesaver,
				lang
			)
		};
		...
		const generateEmbedForFields = async (
			...
			const embed = new EmbedBuilder()
				...
				.setDescription(
					lang.guildprofil_embed_description.replace(
		...
		generateEmbedForFields([ticketFetchedField, reactionRoleField]);
		...
		generateEmbedForFields([xpStatsField, logsField, blockBotField]);
		...
		const collector = interaction.channel?.createMessageComponentCollector({
			filter: (i) =>
				originalResponse.id == i.message.id && i.user.id === i.user.id,
			time: 60_000 * 15
		});
		...
		collector?.on("collect", async (interaction) => {
			...
			if (
				interaction.customId === "guildconf_before" &&
				currentPage > 0
			) {
				currentPage--;
				await editCurrentPage();
			} else if (
				interaction.customId === "guildconf_after" &&
...
function supportConfigToString(
	supportConfig: DatabaseStructure.DbGuildObject["SUPPORT"],
	lang: LanguageData
): string {
	return supportConfig
		? lang.guildprofil_set_supportConfig
				.replace(/\${supportConfig\.input}/g, supportConfig.input ?? "")
				.replace(
					/\${supportConfig\.rolesId}/g,
					supportConfig.rolesId ?? ""
				)
		: lang.guildprofil_not_set_supportConfig;
}

function ticketFetchedToString(
	charForTicket: DatabaseStructure.DbGuildObject["TICKET"],
	lang: LanguageData
): string {
	let ticketFetched = "";
...
function reactionRolesToString(
	charForRr: DatabaseStructure.DbGuildObject["REACTION_ROLES"],
	guild: Guild,
	lang: LanguageData
): string {
	let reactionrole = "";
...
function xpStatsToString(
	xp: DatabaseStructure.DbGuildXpLeveling | undefined,
	lang: LanguageData
): string {
	return xp?.disable === false
		? xp?.xpchannels
			? lang.guildprofil_another_enable_xp.replace(
					"${xp.xpchannels}",
					xp.xpchannels
				)
			: lang.guildprofil_enable_xp
		: lang.guildprofil_disable_xp;
}

function logsToString(
	logs: DatabaseStructure.DbGuildObject["SERVER_LOGS"],
	lang: LanguageData
): string {
	return logs
		? [
				...
				.map((log) => `<#${log}>`)
				.join(",")
		: lang.guildprofil_not_logs_set;
}

function blockBotToString(
	blockBot: DatabaseStructure.DbGuildObject["BLOCK_BOT"],
	lang: LanguageData
): string {
	return blockBot
		? lang.guildprofil_blockbot_on
		: lang.guildprofil_blockbot_off;
}

function picOnlyChannelsToString(
	picOnly: DatabaseStructure.UtilsData["picOnly"] | undefined,
	lang: LanguageData
): string {
	return picOnly === undefined || picOnly.length === 0
		? lang.var_none
		: picOnly?.map((x: string) => `<#${x}>`).join(",");
}

function picOnlyConfigToString(
	picOnlyConfig: DatabaseStructure.PicOnlyConfig | undefined,
	lang: LanguageData
): string {
...
function wlRolesToString(
	wlRoles: DatabaseStructure.UtilsData["wlRoles"] | undefined,
	lang: LanguageData
): string {
	return wlRoles === undefined || wlRoles.length === 0
		? lang.var_none
		: lang.utils_wlroles_embed_desc +
				"\n" +
				wlRoles?.map((x: string) => `<@&${x}>`).join(",");
}

function joinGhostpingToString(
	gp: DatabaseStructure.GhostPingData | undefined,
	lang: LanguageData
): string {
	return gp?.channels === undefined || gp.channels.length === 0
		? lang.var_none
		: lang.joinghostping_add_ok_embed_desc +
				"\n" +
				gp.channels?.map((x: string) => `<#${x}>`).join(",");
}

function antispamToString(
	antispamConfig: AntiSpam.AntiSpamOptions | undefined,
	lang: LanguageData
): string {
	return antispamConfig === undefined
		? lang.var_none
		: `
...
function tooNewAccountToString(
	user: GuildMember,
	tooNewConfig: DatabaseStructure.BlockNewAccountSchema | undefined,
	lang: LanguageData
): string {
	return tooNewConfig?.state === true
...
function roleSaverToString(
	rs: DatabaseStructure.RoleSaverSchema | undefined,
	lang: LanguageData
): string {
	return rs === undefined || rs.enable === false
		? lang.var_none
		: `
...
function pfpsToString(
	pfps: DatabaseStructure.DbInId["PFPS"] | undefined,
	lang: LanguageData
) {
	return pfps === undefined || pfps.disable === true
		? lang.var_no_set
		: `\- ${lang.var_text_channel}: <#${pfps.channel}>`;
}

function securityToString(
	sec: DatabaseStructure.DbInId["SECURITY"] | undefined,
	lang: LanguageData
) {
	return sec === undefined || sec.disable === true
		? lang.var_no_set
		: `
...
function voiceDashToString(
	vc: DatabaseStructure.DbInId["VOICE_INTERFACE"] | undefined,
	lang: LanguageData
) {
	...
	return vc === undefined || !vc.voice_channel
		? lang.var_no_set
		: `
