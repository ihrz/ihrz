...
export const subCommand: SubCommand = {
	run: async (
		...
		let staff_roles: string[] =
			(await client.db.get(
				`${interaction.guildId}.VOICE_INTERFACE.staff_role`
			)) || [];
		...
		const embed = new EmbedBuilder()
			...
			.addFields({
				name:
					lang.setjoinroles_help_embed_fields_1_name || "Staff Roles",
				value:
			...
			async (buttonInteraction: ButtonInteraction) => {
				...
				if (
					...
					await client.func.ihorizon_logs(interaction, {
						title:
							lang.setjoinroles_logs_embed_title_on_enable ||
							"Staff Role Updated",
						description: (
							lang.setjoinroles_logs_embed_description_on_enable ||
