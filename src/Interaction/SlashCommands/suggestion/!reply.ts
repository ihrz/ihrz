...
export const subCommand: SubCommand = {
	run: async (client: Client, interaction: ChatInputCommandInteraction<"cached">, lang: LanguageData, args?: string[]) => {
		...
		if (!fetchId) {
			await interaction.deleteReply();
			await interaction.followUp({ content: lang.suggest_replynot_found_db, flags: [1 << 6] });
			return;
		} else if (fetchId.replied) {
			await interaction.deleteReply();
			await interaction.followUp({ content: lang.suggest_reply_already_replied, flags: [1 << 6] });
			return;
		};
		...
		await (channel as BaseGuildTextChannel).messages.fetch(fetchId?.msgId).then(async (msg) => {
			...
			await interaction.deleteReply();
			await interaction.followUp({ content: lang.suggest_reply_command_error, flags: [1 << 6] });
			return;
		});
	},
};