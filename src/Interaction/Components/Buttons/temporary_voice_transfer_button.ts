...
export default async function (interaction: ButtonInteraction<"cached">) {
	...
	if (!member.voice.channel) {
			...
			components: [
				new ActionRowBuilder<UserSelectMenuBuilder>().addComponents(
					new UserSelectMenuBuilder()
						...
						.setPlaceholder(
							lang.temporary_voice_transfer_menu_placeholder
						)
						.setMinValues(1)
						.setMaxValues(1)
				)
		...
		collector?.on("collect", async (i) => {
			...
			const newOwnerChannelId = (await tempTable.get(
				`CUSTOM_VOICE.${interaction.guildId}.${newOwner?.user?.id}`
			...
			if (newOwnerChannelId && newOwnerChannelId !== getChannelId) {
				const newOwnerChannel =
					interaction.guild.channels.cache.get(newOwnerChannelId) ||
					(await interaction.guild.channels
						.fetch(newOwnerChannelId)
						.catch(() => null));

				if (
					newOwnerChannel?.type === ChannelType.GuildVoice &&
					newOwnerChannel.members.size > 0
				) {
					await i.deferUpdate();
					return;
				}
			...
			if (result?.voice_channel_name) {
				targetedChannel?.setName(
					result.voice_channel_name.includes("{Username}")
						? result.voice_channel_name.replace(
								"{Username}",
								username!
							)
