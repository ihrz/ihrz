...
export const command: Command = {
	...
	run: async (
		...
		let autoreactConfig =
			(await client.db.get(`${interaction.guild.id}.GUILD.AUTOREACT`)) ||
			({} as DatabaseStructure.DbGuildAutoReact);
		...
		const createRankRolesEmbed = async (page: number) => {
			...
			const pageValue = channelEntries.slice(
				startIndex,
			...
			const totalPage = Math.max(
				1,
				Math.ceil(channelEntries.length / itemsPerPage)
			);
			...
			const navigationRow =
				new ActionRowBuilder<ButtonBuilder>().addComponents(
					...
					new ButtonBuilder()
						...
						.setDisabled(
							currentPage >=
								Math.floor(valueEntries.length / itemsPerPage)
						)
				);
		...
		collector.on("collect", async (interaction2) => {
			...
			if (interaction2.customId === "add_channel") {
				...
				const reactInput =
					response2?.fields.getTextInputValue("reaction_input") || "";

				if (reactInput !== "") {
					if (
						!isSingleEmoji(reactInput) &&
						!isDiscordEmoji(reactInput)
					) {
				...
				if (selectedChannel) {
					const existingReactions =
						autoreactConfig[selectedChannel] || [];
					const reactionsArray = Array.isArray(existingReactions)
						? existingReactions
						: [existingReactions];
					...
					if (!reactionsArray.includes(reactInput)) {
						...
						await client.db.set(
							`${interaction.guild?.id}.GUILD.AUTOREACT`,
							autoreactConfig
						);
					}
			...
			if (interaction2.customId === "remove_channel") {
				...
				const channelOptions = Object.entries(
					...
					new StringSelectMenuOptionBuilder()
						.setLabel(
							lang.var_text_channel +
								": " +
								interaction.guild?.channels.cache.get(channelId)
									?.name || lang.var_unknown
						)
						.setValue(channelId)
				);
