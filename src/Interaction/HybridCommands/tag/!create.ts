...
export const subCommand: SubCommand = {
	run: async (
		...
		if (interaction instanceof ChatInputCommandInteraction) {
			...
			var message_content = interaction.options.getString(
				"message_content",
				false
			);
		} else {
		...
		const is_in_wl = interaction.member.roles.cache.some((role) =>
			baseData?.whitelist_create?.includes(role.id)
		);
		...
		await client.db.set(
			`${interaction.guildId}.GUILD.TAGS.storedTags.${tag_name}`,
			{
				embedId: embed_id,
		...
		await client.func.method.interactionSend(interaction, {
			content: lang.tag_create_command_work.replace(
				"${tag_name}",
				tag_name
			)
		});
