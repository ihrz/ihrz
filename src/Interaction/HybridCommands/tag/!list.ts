...
export const subCommand: SubCommand = {
	run: async (
		...
		const baseData = (await client.db.get(
			`${interaction.guildId}.GUILD.TAGS.storedTags`
		)) as DatabaseStructure.GuildTagsStructure["storedTags"] | undefined;

		// Check if there are no tags
		if (!baseData || Object.entries(baseData).length === 0) {
			await client.func.method.interactionSend(interaction, {
				content: lang.tag_list_no_anything
			});
			return;
		}
		...
		const getButtons = (currentPage: number) => {
			return new ActionRowBuilder<ButtonBuilder>().addComponents(
				...
				new ButtonBuilder()
					.setCustomId("page")
					.setLabel(
						`${lang.var_page} ${currentPage + 1}/${arrayEmbeds.length}`
					)
