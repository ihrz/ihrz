...
function generateSelectMenu(
	data: DatabaseStructure.RoleReactData,
	messageId: string,
	placeholder: string
) {
	...
	data.forEach((item, index) => {
		...
		if (
			item.emoji &&
			(isSingleEmoji(item.emoji) || isDiscordEmoji(item.emoji))
		) {
	...
	run: async (
		...
		const channel =
			interaction instanceof ChatInputCommandInteraction
				...
				: ((await client.func.method.channel(
						interaction,
						args!,
						0
					)) as Channel);
		...
		const baseData: DatabaseStructure.RoleReactData =
			(await client.db.get(
				`${interaction.guildId}.GUILD.ROLE_SELECT.${messageId}`
			)) || [];
		...
		function updateConfiguration(data: DatabaseStructure.RoleReactData) {
			...
			if (data.length > 0) {
				components.push(
					new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(
						generateSelectMenu(data, messageId, placeholder)
					)
				);
			}
		}
		...
		const configMessage = await client.func.method.interactionSend(
			interaction,
			{
				embeds: [embed],
		...
		async function handleAddRoleOption(
			interaction2: StringSelectMenuInteraction<CacheType>
		) {
					...
					fields: [
						{
							...
							placeHolder:
								lang.roleselect_modal1_fields1_placeholder,
							maxLength: 120,
							minLength: 1,
							required: false
						},
						{
							...
							minLength: 4,
							placeHolder:
								lang.roleselect_modal1_fields2_placeholder,
							required: true
						},
						{
							...
							placeHolder:
								lang.roleselect_modal1_fields3_placeholder,
							required: false
						}
					],
					deferUpdate: false
			...
			const emoji =
				modal?.fields.getTextInputValue("case_emoji")?.trim() ||
				undefined;
			const label = modal?.fields
				.getTextInputValue("case_title")
				?.trim()!;
			const desc =
				modal?.fields.getTextInputValue("case_desc")?.trim() ||
				undefined;
				...
				components: [
					new ActionRowBuilder<RoleSelectMenuBuilder>().addComponents(
						new RoleSelectMenuBuilder()
							...
							.setPlaceholder(
								lang.roleselect_awaiting1_menu_placeholder
							)
							.setMaxValues(1)
					)
				],
		...
		async function handleRemoveRoleOption(
			interaction2: StringSelectMenuInteraction<CacheType>
		) {
		...
		async function handleCancelConfiguration(
			interaction2: StringSelectMenuInteraction<CacheType>
		) {
			await interaction2.reply({
		...
		async function handlePlaceholderConfiguration(
			interaction2: StringSelectMenuInteraction<CacheType>
		) {
