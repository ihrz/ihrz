...
export const subCommand: SubCommand = {
	run: async (
		...
		if (interaction instanceof ChatInputCommandInteraction) {
			...
			member = (client.func.method.member(interaction, args!, 0) ||
				interaction.member) as GuildMember;
		}
		...
		const res = (await client.db.get(
			`${interaction.guildId}.STATS.USER.${member.user.id}`
		...
		const activeVoiceChannels = calculateActiveVoiceChannels(
			res.voices || []
		);
		...
		monthlyMessages.forEach((msg) => {
			const dayIndex = Math.floor(
				(nowTimestamp - msg.sentTimestamp) / 86400000
			);
		...
		(res.voices || []).forEach((voice) => {
			const dayIndex = Math.floor(
				(nowTimestamp - voice.startTimestamp) / 86400000
			);
			if (dayIndex >= 0 && dayIndex < 30) {
				const sessionDuration =
					(voice.endTimestamp - voice.startTimestamp) / 1000 / 60;
		...
		htmlContent = htmlContent
			...
			.replaceAll(
				"{messages_top1}",
				String(getChannelName(interaction.guild, firstActiveChannel))
			)
			.replaceAll(
				"{messages_top2}",
				String(getChannelName(interaction.guild, secondActiveChannel))
			)
			.replaceAll(
				"{messages_top3}",
				String(getChannelName(interaction.guild, thirdActiveChannel))
			)
			.replaceAll(
				"{messages_top1_2}",
				String(
			...
			.replaceAll(
				"{voice_top1}",
				String(
					getChannelName(interaction.guild, firstActiveVoiceChannel)
				)
			)
			.replaceAll(
				"{voice_top2}",
				String(
					getChannelName(interaction.guild, secondActiveVoiceChannel)
				)
			)
			.replaceAll(
				"{voice_top3}",
				String(
					getChannelName(interaction.guild, thirdActiveVoiceChannel)
				)
			)
			.replaceAll(
				"{voice_top1_2}",
