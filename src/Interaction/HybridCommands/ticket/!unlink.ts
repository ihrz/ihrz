...
export const subCommand: SubCommand = {
	run: async (
		...
		if (
			...
			await client.func.method.interactionSend(interaction, {
				content: lang.transript_not_in_ticket
			});
			return;
		}
		...
		await ticketChannel.setName(
			ticketChannel.name.replace("ticket", "channel")
		);

		let panelMessage = (
			await ticketChannel.messages.fetch({ after: "0", limit: 1 })
		).first();

		if (panelMessage) {
			panelMessage.edit({
				content: lang.ticket_unlink_panel_edited_content.replace(
					"{user}",
		...
		client.func.method.interactionSend(interaction, {
			content: lang.ticket_unlink_command_ok.replace(
