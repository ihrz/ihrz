...
//         this.gameBoard = new Array(42).fill(options.emojis.board);
//     }
...
//             tieMessage: 'La partie est nulle !',
//             timeoutMessage: 'Le jeu est resté inachevé ! Personne n\'a gagné !',
//             requestMessage: '{player} vous a invité à jouer à une partie de **Puissance 4**.',
//             rejectMessage: 'Le joueur a refusé votre demande de partie de **Puissance 4**.',
//             loseMessage: 'Vous avez perdu !',
//             drawMessage: 'La partie est nulle !',
...
//             isSlashGame: this.options.isSlashGame,
//             embed: approvalOptions.embed,
//             buttons: approvalOptions.buttons,
//             reqTimeoutTime: approvalOptions.reqTimeoutTime,
//             requestMessage: approvalOptions.requestMessage,
//             rejectMessage: approvalOptions.rejectMessage,
//             reqTimeoutMessage: approvalOptions.reqTimeoutMessage
//         });

//         const result = await approve.approve();
//         if (result) this.connect4Game(result);
//     }
...
//                 if (this.options.playerOnlyMessage) btn.followUp({ content: this.formatMessage(this.options, 'playerOnlyMessage'), ephemeral: true });
//                 return;
//             }
...
//     getTurnMessage(msg) {
//         return this.formatMessage(this.options, (msg ?? 'turnMessage')).replace('{emoji}', this.getPlayerEmoji());
//     }
...
//         return options[message].replace('{player}', options.message.author.username).replace('{opponent}', options.opponent.username);
//     }

//     formatTurnMessage(options, message) {
//         return options[message].replace('{player}', options.message.author.username).replace('{opponent}', options.opponent.username);
//     }
...
//                 .setDescription(this.formatMessage(this.options, 'requestMessage'));
...
//                     .setDescription(this.formatMessage(this.options, 'rejectMessage'));

//                 if (reason === 'time') embed.setDescription(this.formatMessage(this.options, 'reqTimeoutMessage'));
