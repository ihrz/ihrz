...
export const command: Command = {
    ...
    aliases: ["invitesremove", "invites-remove", "remove-invites"],
    ...
    run: async (client: Client, interaction: Message, execTimestamp: number, args: string[]) => {
        ...
        let amount = client.func.arg.number(interaction, 1) as number;
        ...
        if (!interaction.member.permissions?.has(PermissionsBitField.Flags.Administrator)) {
            await interaction.reply({ embeds: [a], allowedMentions: { repliedUser: false } });
            return;
        }
