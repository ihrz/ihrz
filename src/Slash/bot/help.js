...
const { EmbedBuilder, ActionRowBuilder, ComponentType, StringSelectMenuBuilder, StringSelectMenuOptionBuilder } = require(`${process.cwd()}/files/ihorizonjs`);
...
slashInfo.bot.help.run = async (client, interaction) => {
  ...
  const categories = [
    ...
    { name: data.help_roler_fields, value: CONTENT.rolereaction, inline: true, description: data.help_roler_dsc, emoji: "📇" },
  ...
  collector.on('collect', async i => {
    ...
    await categories[i.values[0]].value.forEach(element => {
