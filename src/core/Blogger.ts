...
export class BloggerNotifier {
	...
	private async getGuildData(guildID: string): Promise<DatabaseStructure.BloggerSchema | null> {
		const all = await client.db.get(`${guildID}.BLOGGER`) as DatabaseStructure.BloggerSchema | null
		return all
	}

	private async getGuildsData(): Promise<{ value: DatabaseStructure.BloggerSchema, guildId: string }[]> {
		...
		return all
			...
			.map(v => {
				...
				return guildObject.BLOGGER ? { value: guildObject.BLOGGER, guildId: v.id } : null;
			})
			.filter(Boolean) as { value: DatabaseStructure.BloggerSchema, guildId: string }[];
	}
	...
	private async fetchBlogsFeeds(blogs: DatabaseStructure.BloggerBlogSchema[]): Promise<BloggerFeedResponse[]> {
		...
		for (const blog of blogs) {
			try {
				...
				logger.err(`Erreur lors de la vérification du flux RSS pour ${blog.rss} : ${error}`);
			}
	...
	private async articleHaveAlreadyBeNotified(guildID: string, article: BloggerFeedResponse): Promise<boolean> {
		const notifiedArticles = (await client.db.get(`${guildID}.BLOGGER.lastArticleNotified`) || []) as DatabaseStructure.BloggerLastNotifiedArticles[];
		return notifiedArticles.some(item =>
			item.blogId === article.blog.id &&
			(item.articleId === article.content.id || new Date(item.timestamp) >= new Date(article.content.pubDate))
		);
	}
	...
	public async blogExist(guildId: string, blogId: string): Promise<boolean> {
		const allGuildsData = await client.db.get(`${guildId}.BLOGGER`) as DatabaseStructure.BloggerSchema;
	...
	public async validateRssFeed(rssUrl: string): Promise<{ valid: boolean, name?: string }> {
		try {
			const feed = await this.parser.parseURL(rssUrl);
	...
	public async generateBlogsEmbed(guild: Guild): Promise<EmbedBuilder> {
		...
		for (const blog of blogs) {
			...
			const channel = await guild.channels.fetch(blog.channelId).catch(() => null);
	...
	private async refresh() {
		...
		for (const entry of guildsData) {
			...
			const lang = await client.func.getLanguageData(guild?.id);
			const articles = await this.fetchBlogsFeeds(entry.value.blogs || []);

			for (const article of articles) {
				if (!await this.articleHaveAlreadyBeNotified(entry.guildId, article)) {
					const channel = await guild?.channels.fetch(article.blog.channelId).catch(() => null) as BaseGuildTextChannel | undefined;
					...
					if (channel) {
						const message = client.func.method.generateCustomMessagePreview(
							lang.blogger_on_new_article_default_message,
							{
								...
								blogger: {
									...
									blogName: await this.getBlogNameByRss(article.blog.rss)
								}
							}
						);

						await client.db.push(`${entry.guildId}.BLOGGER.lastArticleNotified`, {
						...
						await channel.send({
							content: message,
							components: [this.createLinkButton(article.content.link, lang.blogger_read_article)],
	...
	public async start() {
		logger.log("Starting Blogger RSS Notifier...");
