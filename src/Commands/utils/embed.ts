                ...
                case '12':
                    ...
                    colorCollector.on('collect', async (message: { content: string | number | readonly [red: number, green: number, blue: number] | null; delete: () => any; }) => {
                        if (reg.test((message.content as any))) {
                            __tempEmbed.setColor(await client.db.get(`${interaction.guild.id}.GUILD.GUILD_CONFIG.embed_color.all`) || (message.content as any));
                            response.edit({ embeds: [__tempEmbed] });
                        } else {
                    ...
                    break;
                case '13':
                    __tempEmbed.setColor(await client.db.get(`${interaction.guild.id}.GUILD.GUILD_CONFIG.embed_color.all`) || "#000000");
                    response.edit({ embeds: [__tempEmbed] });
