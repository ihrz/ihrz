...
export = {
    run: async (client: Client, interaction: any, data: any) => {
        ...
        if (!fetchId) {
            await interaction.editReply({ content: data.suggest_replynot_found_db, ephemeral: true });
            return;
        } else if (fetchId.replied) {
            await interaction.editReply({ content: data.suggest_reply_already_replied, ephemeral: true });
            return;
        };
        ...
        await channel.messages.fetch(fetchId?.msgId).then(async (msg: any) => {
            ...
            await interaction.editReply({ content: data.suggest_reply_command_error, ephemeral: true });
            return;
        });
    },
};