...
export = {
    run: async (client: Client, interaction: any, data: any) => {
        ...
        var level: Number = await db.get(`${interaction.guild.id}.USER.${member.id}.XP_LEVELING.level`);
        if (!level) var level: Number = 0;

        var balance: Number = await db.get(`${interaction.guild.id}.USER.${member.id}.ECONOMY.money`);
