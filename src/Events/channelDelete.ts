...
export = async (client: Client, channel: any) => {
    ...
    async function protect() {
        let data = await db.get(`${channel.guild.id}.PROTECTION`);
        ...
        if (data.deletechannel && data.deletechannel.mode === 'allowlist') {
            ...
            let baseData = await db.get(`${channel.guild.id}.ALLOWLIST.list.${firstEntry.executorId}`);
    ...
    async function ticketModule() {
        let fetch = await db.get(`${channel.guild.id}.TICKET_ALL`);

        for (let user in fetch) {
            for (let channel_2 in fetch[user]) {

                if (channel.id === channel_2) {
                    await db.delete(`${channel.guild.id}.TICKET_ALL.${user}`);
                }
            }
        }
    };
