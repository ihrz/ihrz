...
export const event: BotEvent = {
	...
	run: async (client: Client, oldGuild: Guild, newGuild: Guild) => {
		...
		if (
			!oldGuild.members.me?.permissions.has([
		...
		if (data.updateguild) {
			...
			if (data.updateguild.mode === "allowlist") {
				const baseData = await client.db.get(
					`${newGuild.id}.ALLOWLIST.list.${relevantLog.executorId}`
				);

				if (!baseData) {
					user =
						newGuild.members.cache.get(
			...
			!isOwner &&
				...
				(async () => {
					...
					if (oldGuild.afkChannel !== newGuild.afkChannel) {
						await newGuild
							.setAFKChannel(oldGuild.afkChannel)
							.catch(() => false);
					}
					if (oldGuild.afkTimeout !== newGuild.afkTimeout) {
						await newGuild
							.setAFKTimeout(oldGuild.afkTimeout)
							.catch(() => false);
					}
					if (oldGuild.banner !== newGuild.banner) {
						await newGuild
							.setBanner(oldGuild.banner)
							.catch(() => false);
					}
					if (
					...
					if (oldGuild.discoverySplash !== newGuild.discoverySplash) {
						await newGuild
							.setDiscoverySplash(oldGuild.discoverySplash)
							.catch(() => false);
					}
					if (
					...
					if (oldGuild.icon !== newGuild.icon) {
						await newGuild
							.setIcon(oldGuild.icon)
							.catch(() => false);
					}
					if (oldGuild.mfaLevel !== newGuild.mfaLevel) {
						await newGuild
							.setMFALevel(oldGuild.mfaLevel)
							.catch(() => false);
					}
					if (oldGuild.name !== newGuild.name) {
						await newGuild
							.setName(oldGuild.name)
							.catch(() => false);
					}
					if (oldGuild.preferredLocale !== newGuild.preferredLocale) {
						await newGuild
							.setPreferredLocale(oldGuild.preferredLocale)
							.catch(() => false);
					}
					if (
