...
export const event: BotEvent = {
	...
	run: async (client: Client, reaction: MessageReaction, user: User) => {
		try {
			...
			let skullboardData: DatabaseStructure.StarboardDataSchema =
				(await client.db.get(skullboardDataPath)) || [];

			if (
				!skullboardChannel ||
				!(skullboardChannel instanceof TextChannel)
			) {
				return;
			}
			...
			const embed = new EmbedBuilder()
				...
				.setAuthor({
					...
					iconURL:
						reaction.message.author?.displayAvatarURL() || undefined
				})
				.setDescription(
				...
				.setFooter(
					await client.func.displayBotName.footerBuilder(guildId)
				);
			...
			if (existingEntry) {
				...
				try {
					const skullboardMessage = await skullboardChannel.messages
				...
				try {
					...
					if (baseData.createThread) {
						await skullboardMessage
							.startThread({
								name:
									"💀 " +
									lang.var_s_message.replace(
										"{nickname}",
			...
			console.error(
				"Error in messageReactionAdd skullboard event:",
				error
			);
		}
	}
