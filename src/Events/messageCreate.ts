...
export = async (client: Client, message: any) => {
    ...
    async function xpFetcher() {
        ...
        var baseData = await db.get(`${message.guild.id}.USER.${message.author.id}.XP_LEVELING`);
        var xp = baseData?.xp;
        var level = baseData?.level || 1;

        if ((level * 500) < xp) {
            await db.add(`${message.guild.id}.USER.${message.author.id}.XP_LEVELING.level`, 1);
            await db.sub(`${message.guild.id}.USER.${message.author.id}.XP_LEVELING.xp`, (level * 500));

            let newLevel = await db.get(`${message.guild.id}.USER.${message.author.id}.XP_LEVELING.level`);

            let xpTurn = await db.get(`${message.guild.id}.GUILD.XP_LEVELING.disable`);
            ...
            try {
                ...
                for (let word of blacklist) {
                    if (contentLower.includes(word)) {
                        let FLAGS_FETCH = await db.get(`TEMP.${message.guild.id}.PUNISH_DATA.${message.author.id}.flags`);
                        FLAGS_FETCH = FLAGS_FETCH || 0;

                        await db.set(`TEMP.${message.guild.id}.PUNISH_DATA.${message.author.id}`, { flags: FLAGS_FETCH + 1 });
    ...
    async function createAllowList() {
        let baseData = await db.get(`${message.guild.id}.ALLOWLIST`);

        if (!baseData) {
            await db.set(`${message.guild.id}.ALLOWLIST`,
                {
                    enable: false,
    ...
    async function suggestion() {
        let baseData = await db.get(`${message.guild.id}.SUGGEST`);
        ...
        await db.set(`${message.guild.id}.SUGGESTION.${suggestCode}`,
            {
