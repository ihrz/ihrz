...
export const event: BotEvent = {
	...
	run: async (client: Client, message: Message) => {
		...
		const guildLocal = await client.db.get(`${message.guild.id}.GUILD.LANG.lang`) || "en-US";
		...
		const baseData = await client.db.get(`${message.guild.id}.USER.${message.author.id}.XP_LEVELING`) as DatabaseStructure.XpLevelingUserSchema;
		const ranksConfig = await client.db.get(`${message.guild.id}.GUILD.XP_LEVELING`) as DatabaseStructure.DbGuildObject['XP_LEVELING'];
		...
		if ((level * 500) < baseData?.xp!) {
			...
			const MsgChannel = message.guild.channels.cache.get(xpChan) as GuildTextBasedChannel | null;
